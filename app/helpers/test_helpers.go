...
func setupNoValset(withGenesis bool, invCheckPeriod uint) (*govgenapp.GovGenApp, govgenapp.GenesisState) {
	db := dbm.NewMemDB()
	encCdc := govgenapp.MakeTestEncodingConfig()
	app := govgenapp.NewGovGenApp(log.NewNopLogger(), db, nil, true, map[int64]bool{}, govgenapp.DefaultNodeHome, invCheckPeriod, encCdc, EmptyAppOptions{})
	if withGenesis {
		return app, govgenapp.NewDefaultGenesisState()
	}
	return app, govgenapp.GenesisState{}
}

func SetupNoValset(isCheckTx bool) *govgenapp.GovGenApp {
	app, genesisState := setupNoValset(!isCheckTx, 5)
	if !isCheckTx {
...
func initAccountWithCoins(app *govgenapp.GovGenApp, ctx sdk.Context, addr sdk.AccAddress, coins sdk.Coins) {
